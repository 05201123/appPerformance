...
/*	private void cancelTaskAtDialogDismiss(){
	...
	public int getTaskSize(){
		return waitTasks.size()+processTasks.size();
	}
	public void trimSize(int taskSize){
		synchronized (waitTasks) {
			if(waitTasks.size()>taskSize)
			{
				List<BaseTask> subList = new Vector<BaseTask>();
				for(int index=0;index<taskSize;index++){
					subList.add( waitTasks.poll());
				}
				waitTasks.clear();
				waitTasks.addAll(subList);
			}
